...
- task 1 finding the most frequent mutation
- task 2
- task 3
- task N

```{r}
names(sort(table(as.factor(d$p.Protein)), decreasing=T))
sort(table(as.factor(d$p.Protein)), decreasing=T)
```
...
p_i <- gsub('[^0-9]', '', d$p.Protein)
p_i
sort(table(as.factor(p_i)), decreasing=T)
table(as.factor(p_i))
```
