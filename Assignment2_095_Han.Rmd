...
## 데이터셋 확인
우선 데이터를 로드해주자. 데이터셋에 존재하는 세 data frame을 각각 임시 data frame으로 옮긴다.   
...
Column을 time(week) gene1 gene2... 순으로 만들어주어야 한다. 지금은 Column에 sample들이 위차한다.   
우선 row와 column을 뒤집어줄 필요가 있다.
그 후 total data frame을 만든다.

```{r}
exp <- exp %>% 
  mutate(gene_symbol = gen$ensembl_gene_id) %>%
  .[, c(36, 1:35)]

tot <- data.frame(t(exp[-1]))
colnames(tot) <- exp[, 1]
tot <- tot %>% mutate(age_week = sam$age_week)
```
...
```{r}
doi <- tot %>% select(ENSG00000007372, ENSG00000178403, ENSG00000139352, ENSG00000134323, ENSG00000096968, ENSG00000168610, ENSG00000162599, age_week)
```
...
p <- doi %>% filter(age_week <= 55) %>%
  ...
  geom_point(aes(y = ENSG00000007372, col = "Pax6"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000007372, col = "Pax6", linetype = "Onset"), size=0.4, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE) + 
  geom_point(aes(y = ENSG00000178403, col = "Ngn2"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000178403, col = "Ngn2", linetype = "Onset"), size=0.4, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE) +
  geom_point(aes(y = ENSG00000139352, col = "Ascl1"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000139352, col = "Ascl1", linetype = "Onset"), size=0.4, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE) +
    geom_point(aes(y = ENSG00000134323, col = "Myc"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000134323, col = "Myc", linetype = "Progression"), size=0.7, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE) +
    geom_point(aes(y = ENSG00000096968, col = "Jak"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000096968, col = "Jak", linetype = "Termination"), size=1.2, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE) +
    geom_point(aes(y = ENSG00000168610, col = "Stat"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000168610, col = "Stat", linetype = "Termination"), size=1, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE) +
    geom_point(aes(y = ENSG00000162599, col = "Nf1a"), size = 0.3, alpha = 0.2) +
  stat_smooth(aes(y = ENSG00000162599, col = "Nf1a", linetype = "Termination"), size=1, method = "loess", level = 0.9, fullrange = TRUE, se = FALSE, span = 2/3, na.rm = TRUE)
p
```

stage를 표시하는 점선을 그리고 stage를 표시한다.
```{r}
p <- p + 
...
```{r}
p <- p + 
  scale_color_locuszoom(name = "Genes", breaks=c("Pax6","Ngn2","Ascl1","Myc","Jak","Stat","Nf1a")) +
...
Assignment2 : 피드백으로 달라진 점     
우선 불필요한 코드나 데이터 프레임을 삭제하고 필요한 코드만 남겨놓았습니다. 그리고 코멘트에서 두번 지적받은 일부 유전자가 그래프 구분이 안된다는 점을 해결하기 위해 색 팔레트와 두께를 조절했습니다.   
